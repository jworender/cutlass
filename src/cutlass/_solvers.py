...
class _CDLogistic:
    ...
    def fit(
        ...
        *,
        ...
        if all_pm1:
            # When features are all ±1 the diagonal of X^T W X collapses to a single
            # scalar, but we still need a per-feature vector for the coordinate
            # updates and to maintain shared code paths below.
            mean_h = float(np.mean(wght))
            diag_h = np.full(p, max(mean_h, 1e-12), dtype=np.float64)
        else:
            diag_h = np.mean((X ** 2) * wght[:, None], axis=0)
