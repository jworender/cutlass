...
class DataObject:
    ...
    def automate_modeling(self, target_column, experiment_name='auto_ml_experiment', test_size=0.2, fold=5, verbose=True):
        """
        Automate the machine learning workflow using PyCaret.

        :param target_column: The name of the target variable.
        :param experiment_name: Name for the PyCaret experiment.
        :param test_size: Proportion of the dataset to include in the test split.
        :param fold: Number of folds for cross-validation.
        :param verbose: Whether to display PyCaret logs.
        :return: Dictionary of models trained by PyCaret with their respective metrics.
        """
        if self.data is None:
            raise ValueError("No data available for automated modeling.")
        ...
        if y.dtype == 'object' or y.nunique() <= 10:
            ...
            setup_result = pycaret_class_setup(
                data=self.data,
            ...
            # Compare models
            best_model = pycaret_class_compare_models(fold=fold, verbose=verbose)
            # Tune model
            tuned_model = pycaret_class_tune_model(best_model, fold=fold, verbose=verbose)
            # Finalize model
            final_model = pycaret_class_finalize_model(tuned_model, verbose=verbose)
            # Get model details
            metrics_df = pycaret_class_pull()
            metrics = metrics_df.to_dict(orient='records') if metrics_df is not None else {}
        else:
            task = 'regression'
            # Setup PyCaret for regression
            setup_result = pycaret_reg_setup(
                data=self.data,
                target=target_column,
                session_id=42,
                train_size=1 - test_size,
            ...
            # Compare models
            best_model = pycaret_reg_compare_models(fold=fold, verbose=verbose)
            # Tune model
            tuned_model = pycaret_reg_tune_model(best_model, fold=fold, verbose=verbose)
            # Finalize model
            final_model = pycaret_reg_finalize_model(tuned_model, verbose=verbose)
            # Get model details
            metrics_df = pycaret_reg_pull()
            metrics = metrics_df.to_dict(orient='records') if metrics_df is not None else {}

        # Store the model
        model_name = f'pycaret_{task}_model'
        if model_name in self.models:
            print(f"Overwriting existing model with name '{model_name}'.")
        self.models[model_name] = {
            'model': final_model,
            'type': f'pycaret_{task}',
            'task': task,
        ...
        print(f"PyCaret {task.capitalize()} model '{model_name}' trained and stored.")
